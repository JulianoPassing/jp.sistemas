...
/**
 * Gera PKCE (S256). Obrigatório se o app ML tiver PKCE ativado no DevCenter.
 * Ver: AUTOPECASRG_ML_USE_PKCE no servidor.
 */
function generatePkcePair() {
  const crypto = require('crypto');
  const code_verifier = crypto.randomBytes(32).toString('base64url');
  const code_challenge = crypto.createHash('sha256').update(code_verifier, 'utf8').digest('base64url');
  return { code_verifier, code_challenge };
}

function buildAuthUrl({ clientId, redirectUri, state, codeChallenge, codeChallengeMethod }) {
  ...
  if (codeChallenge) {
    ...
    p.set('code_challenge_method', codeChallengeMethod || 'S256');
  }
