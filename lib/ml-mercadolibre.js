...
/** Etiqueta PDF (Mercado Envios). `formato` reservado (pdf_a4, pdf_100x150) — a API pode ignorar; impressão usa a impressora configurada. */
async function getShipmentLabelsPdfBuffer(accessToken, shipmentId, formato) {
  const f = formato && String(formato).trim() ? String(formato).trim() : 'pdf';
