...
async function ensureDatabase() {
  ...
  await migrateEtiquetasFormatoPdf1015(p);
  await migrateVendasEstoqueEtiquetasShopee(p);
  return p;
}

/** Padrão: PDF 10×15 cm (impressora comum), não A4. */
async function migrateEtiquetasFormatoPdf1015(p) {
  try {
    await p.execute(
      `ALTER TABLE config_etiquetas MODIFY COLUMN formato_ml VARCHAR(32) DEFAULT 'pdf_100x150'`
    );
  } catch (e) {
    if (e.code !== 'ER_BAD_FIELD_ERROR') console.warn('[autopecasrg-db] migrate formato_ml:', e.message);
  }
  try {
    await p.execute(
      `ALTER TABLE config_etiquetas MODIFY COLUMN formato_shopee VARCHAR(32) DEFAULT 'pdf_100x150'`
    );
  } catch (e) {
    if (e.code !== 'ER_BAD_FIELD_ERROR') console.warn('[autopecasrg-db] migrate formato_shopee:', e.message);
  }
  try {
    await p.execute(
      `UPDATE config_etiquetas SET formato_ml = 'pdf_100x150' WHERE formato_ml = 'pdf_a4'`
    );
    await p.execute(
      `UPDATE config_etiquetas SET formato_shopee = 'pdf_100x150' WHERE formato_shopee = 'pdf_a4'`
    );
  } catch (e) {
    console.warn('[autopecasrg-db] migrate formato para pdf_100x150:', e.message);
  }
}
...
async function migrateConfigEtiquetasVendas(p) {
  const tries = [
    ...
    'ALTER TABLE config_etiquetas ADD COLUMN formato_ml VARCHAR(32) DEFAULT "pdf_100x150"',
    'ALTER TABLE config_etiquetas ADD COLUMN formato_shopee VARCHAR(32) DEFAULT "pdf_100x150"',
...
async function initSchema(p) {
  ...
  await p.execute(`
    CREATE TABLE IF NOT EXISTS config_etiquetas (
      ...
      formato_ml VARCHAR(32) DEFAULT 'pdf_100x150',
      formato_shopee VARCHAR(32) DEFAULT 'pdf_100x150',
