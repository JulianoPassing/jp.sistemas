...
function rotuloFrequenciaJuros(frequencia) {
  const f = String(frequencia || 'monthly').toLowerCase();
  if (f === 'daily') return 'por dia';
  if (f === 'weekly') return 'por semana';
  if (f === 'biweekly') return 'por quinzena';
  return 'por mês';
}
...
Solicitamos o pagamento até a data de vencimento.`;
    ...
    case 'emprestimo_sem_juros': {
      const variaveis = {
        ...
        '{texto_periodos_em_atraso}': dados.textoPeriodosEmAtraso || '',
        '{juros_extra_atraso}': dados.jurosExtraAtraso || dados.jurosAtraso || '',
        '{juros_atraso}': dados.jurosAtraso || dados.jurosExtraAtraso || '',
        '{valor_base_juros}': dados.valorBaseJuros || '',
        '{chave_pix}': chavePix,
        '{nome_banco_pix}': nomeBancoPix
      };
...
📊 *Detalhes:*
• Valor investido: ${utils.formatCurrency(dadosNotificacao.valorInvestido)}
• Juros (${dadosNotificacao.jurosPercent}% ${rotulo}): ${utils.formatCurrency(dadosNotificacao.jurosTotal)}
• Juros extras pelo atraso: ${utils.formatCurrency(dadosNotificacao.jurosAplicado || 0)}
...
💵 *Apenas juros: ${utils.formatCurrency(dadosNotificacao.jurosTotal)}*`;
  ...
  const msgEmprestimoSemJurosPersonalizada = gerarMensagemCobranca('emprestimo_sem_juros', {
    ...
    jurosAtraso: utils.formatCurrency(dadosNotificacao.jurosAplicado || 0),
    jurosExtraAtraso: utils.formatCurrency(dadosNotificacao.jurosAplicado || 0),
    valorBaseJuros: baseJ > 0 ? utils.formatCurrency(baseJ) : '',
    periodosAberto: p,
    textoPeriodosAberto: textoAbertoLinha || '',
      ...
      try {
        ...
        if (parcelas.length > 0) {
          ...
          if (parcelasPagas.length === parcelas.length) {
            ...
            status = 'Em Atraso';
            parcelaMaisAtrasada = parcelasAtrasadas.sort((a, b) =>
        ...
        if (opts.numeroParcela && parcelas.length > 0) {
          ...
          if (parcelaEspecifica) {
            const dataVP = utils.createValidDate(parcelaEspecifica.data_vencimento);
            const parcelaAtrasada = dataVP && dataVP < hoje && parcelaEspecifica.status !== 'Paga';
            let pa = periodosAberto;
        ...
        const detalhes = `
          <div class="emprestimo-modal-box" style="padding: 1.5rem; max-width: 420px; margin: 0 auto; background: #fff; border-radius: 16px; box-shadow: 0 2px 16px #002f4b22;">
            ...
            <div style="margin-bottom: 1.2rem;">
              ...
              <div style="font-size: 1rem; color: #444;">Juros <b>${jurosPercent}%</b> ${rotuloFrequenciaJuros(frequencia)} (${utils.formatCurrency(jurosTotal)})</div>
              ${infoJuros}
              ${emp.observacoes ? `
